...
public class BoardMgr {
	...
	private static final String  SAVEFOLDER = "C:/Jsp/myapp/WebContent/board/fileupload";
	...
	public void insertBoard(HttpServletRequest req) {
		...
		//리퀘스트 밭아온내용 처리 가능 하게 만들어준다.(폼요소 처리가능)
	...
	public void rollback(){
		...
		try {
			con = pool.getConnection();
			sql = "insert tbldelete(name,content,subject,ref,pos,depth,regdate,pass,count,ip)";
			sql += "values(?,?,?,?,?,?,now(),?,0,?)";
			pstmt = con.prepareStatement(sql);
	...
	public void deleteBoard(int num) {
		...
		try {
			...
			sql = "update tblBoard set pass=?,subject=?,content=? where num=?";
			pstmt = con.prepareStatement(sql);
			pstmt.setString(1, "");
			pstmt.setString(2, "삭제된글");
			pstmt.setString(3, "삭제된글");
			pstmt.setInt(4, num);
			pstmt.executeUpdate();
		} catch (Exception e) {
			e.printStackTrace();
